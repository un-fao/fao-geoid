...
- **Base data:** ODbL (`DATA-LICENSE`) — share-alike keeps the federation open *(FAO to confirm)*

## Status

Phase 1 / Release 1 (the June 24 MVP). Authenticated RBAC is a Release-1 **stretch goal**, gated on
FAO's unified authentication service (Eduardo's team) — until then a temporary static admin token gates
