...
- **(=)** Two pre-existing, out-of-scope ceilings carry over **unchanged** from polygons: **3D (Z)
  coordinates** hash distinctly from their 2D form (the recipe keeps Z), and a geometry whose vertices
  all collapse under `1e-7` to nothing would canonicalize to empty — now blocked for every type by
  `ck_place_geom_not_empty`.
