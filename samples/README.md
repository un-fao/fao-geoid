...
| `duplicate_of_GH-COCOA-001.geojson` | The same geometry as GH-COCOA-001 but reversed winding and no id → **duplicate rejected** (POST returns 409 — geometry uniqueness is catalog-wide; the body names the incumbent geoid because the `public` collection is `public_read` — a private incumbent would be masked for non-members). |
